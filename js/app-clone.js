...
document.addEventListener('DOMContentLoaded', () => {
                ...
                $("main").load('./view/playmusic.php', function () {
                    ...
                    $.get('./controller/select_data.php', { 'key': "getAllData" }, function (response) {
                        ...
                        if (res.error !== 1) {
                            ...
                            // })
                            load_music(datas, id_artist, document.querySelector('#careMusic'));
    ...
    function load_name_artist(id_artist, music_id = -1, name_other_artist) {
        ...
        if (Array.isArray(id_artist)) {
            id_artist_temp.forEach((ar_id, index, arr) => {
                if (arr[index].m_id == music_id) {
                    if (index + 1 <= arr.length - 1 && arr[index + 1].m_id == music_id) {
                        ...
                        id_artist_temp = id_artist_temp.filter(id_artist => id_artist.m_id == music_id);
                    } else {
                        html += `<span class="name_artist" id_artist="${ar_id.ar_id}"> ${ar_id.name_artist.trim()}</span>` + " ";
                        id_artist_temp = id_artist_temp.filter(id_artist => id_artist.m_id == music_id);
                    }
        ...
        if (uID) {
            ...
            $.get("./controller/select_data.php", data, function (response) {
                ...
                if ($(_this).attr('id') == "imgUser" && res.error !== 1) {
                    ...
                    load_music(data, id_artist, document.querySelector(".update_profile .playlist ul"), true, null);
    ...
    function load_layout_music_hot(datas, id_artist) {
        let html = datas.map((data, index) => {
            return `
                    ...
                    <div class="imageMusic">
                    <img src="${data.img}" alt="">
                    <div class="playMusic ${currentId === data.m_id && isPlaying ? "active" : ""}">
                        <ion-icon name="play"></ion-icon>
                    </div>
                    <div class="runAudio ${currentId === data.m_id && isPlaying ? "active" : ""}">
                        <div><span></span><span></span><span></span><span></span></div>
                    </div>
                    </div>
                    <div class="desMusic">
